...
class InventoryService extends BaseService<Inventory> {
  ...
  // Sobrescribir downloadExcel para incluir nombre de sucursal
  async downloadExcel(params: FilterParams = {}, storeName?: string): Promise<void> {
    try {
      ...
      link.setAttribute('download', `${storePrefix}${this.downloadFileName}_${date}.xlsx`);
  ...
  // Descarga PDF planilla de inventario
  async downloadPdf(params: FilterParams = {}, storeName?: string): Promise<void> {
    try {
