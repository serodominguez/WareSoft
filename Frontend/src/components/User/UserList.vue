    ...
    <v-card elevation="2">
      <v-data-table-server :headers="headers" :items="users" :search="search || undefined" :items-per-page-text="pages"
        ...
        <template v-slot:no-data>
          <v-btn color="primary" @click="$emit('fetch-users')"> Reset </v-btn>
        </template>
      </v-data-table-server>
    </v-card>
    <UserFilters v-model="drawerModel" v-model:selected-filter="selectedFilterModel" v-model:state="stateModel"
