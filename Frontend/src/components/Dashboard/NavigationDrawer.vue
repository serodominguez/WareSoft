...
const visibleConfigurationLinks = computed((): Link[] => 
  linkConfiguration.filter(link => hasModuleAccess(link.module))
);
...
const hasConfigurationPermissions = computed((): boolean => 
  visibleConfigurationLinks.value.length > 0
);
</script>